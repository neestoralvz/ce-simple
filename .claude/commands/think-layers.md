...
1. **LAYER ASSESSMENT**: Determine appropriate starting and ending layers
2. **COGNITIVE ESCALATION**: Systematically increase analysis depth
3. **INTEGRATION CASCADE**: Build insights across all thinking levels
4. **SYNTHESIS GENERATION**: Consolidate findings into actionable intelligence
5. **EXECUTION PLANNING**: Convert analysis into implementable steps
...
### Input Triggers
**Context**: Complex analysis or synthesis required
**Previous**: `/explore-codebase` → Analysis of discoveries OR `/explore-web` → Research synthesis
**Keywords**: analyze, think, understand, synthesize, evaluate, assess

### Output Triggers
**When**: Analysis complete → Automatic plan consolidation and agent delegation
**When**: Documentation tasks detected → Agent execution of `/docs-workflow` with todo plans
**When**: Implementation roadmap ready → Main workflow execution with non-docs tasks
**When**: Insights require validation → `/explore-web` for additional research
**Chain**: think-layers → {docs-agent + main-execution} → capture-learnings

### Success Patterns
**Analysis Success**: Clear understanding achieved → Automatic plan consolidation triggered
**Synthesis Success**: Implementation roadmap generated → Task division and agent delegation
**Consolidation Success**: Documentation tasks → Agent workflow, execution tasks → Main workflow
**Integration Success**: Parallel workflows coordinated → Ready for combined execution

## 🔗 See Also

### Related Commands
- Execute `/start` to initiate workflows that trigger progressive analysis
- Execute `/explore-codebase` and `/explore-web` to generate input for analysis
- Execute `/docs-workflow` via automatic agent delegation for documentation tasks
- Execute `/capture-learnings` for post-execution pattern detection and integration

### Integration Points
- Core analytical engine processing discoveries from exploration commands
- Progressive cognitive escalation adapts to problem complexity automatically
- Cross-layer validation ensures consistency and logical reasoning
- Automatic plan consolidation divides implementation roadmap into task categories
- Documentation tasks automatically delegated to independent agent workflows
- Parallel execution coordination between docs agent and main implementation workflow

---

**CRITICAL**: This command provides the core analytical engine with automatic plan consolidation for the entire system. Progressive thinking layers ensure appropriate cognitive depth for problem complexity, followed by intelligent task division and automatic documentation workflow delegation.