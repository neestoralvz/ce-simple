...
BASH_CALLS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -l "Bash\|bash tool\|executing.*script\|tool call.*bash" 2>/dev/null | wc -l)
READ_CALLS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -l "Read\|reading file\|file content\|read tool" 2>/dev/null | wc -l)
WRITE_CALLS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -l "Write\|writing to\|file created\|write tool" 2>/dev/null | wc -l)
EDIT_CALLS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -l "Edit\|editing\|file updated\|edit tool" 2>/dev/null | wc -l)
...
ACTUAL_SCRIPT_EXECUTIONS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -c "executed.*\.sh\|script.*completed\|bash.*output" 2>/dev/null | awk -F: '{sum += $2} END {print sum}')
...
VISUAL_ANNOUNCEMENTS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -c "║\|╔\|╚\|EXECUTING\|ACTIVE TOOL CALL" 2>/dev/null | awk -F: '{sum += $2} END {print sum}')
COMMAND_TRANSPARENCY=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -c "Command:.*executed\|/.*command.*activated" 2>/dev/null | awk -F: '{sum += $2} END {print sum}')

# Simulation detection
SIMULATION_PATTERNS=$(find scripts/results operations/outputs -name "*.md" -o -name "*.json" -o -name "*.log" | xargs grep -c "would execute\|recommend running\|you should.*run\|could run\|might want to.*script" 2>/dev/null | awk -F: '{sum += $2} END {print sum}')
