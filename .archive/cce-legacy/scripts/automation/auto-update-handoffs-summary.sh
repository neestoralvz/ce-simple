...
# 
# Description:
# - Scans all handoff files in /operations/handoffs/
...
HANDOFFS_DIR="${PROJECT_ROOT}/operations/handoffs"
...
git_integration() {
    ...
    if [[ "$mode" == "hook" ]]; then
        ...
        if git diff --cached --name-only | grep -q "operations/handoffs/"; then
            handoffs_changed=true
        fi
        
        # Check for modified active handoffs
        if git diff --cached --name-only | grep -q "operations/handoffs/active/"; then
            handoffs_changed=true
        fi
