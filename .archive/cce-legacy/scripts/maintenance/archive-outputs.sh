...
OUTPUTS_DIR="$PROJECT_ROOT/operations/outputs"
...
archive_analysis_files() {
    log "Checking operations/outputs/analysis for files older than $ARCHIVE_DAYS days..."
    
    if [ ! -d "$OUTPUTS_DIR/analysis" ]; then
        warning "operations/outputs/analysis directory not found, skipping..."
        return
    fi
...
### **Outputs Archive**
- **Location**: \`operations/outputs/archive/$CURRENT_YEAR/\`
...
\`\`\`
operations/outputs/archive/$CURRENT_YEAR/           ← Analysis files
scripts/results/archive/
