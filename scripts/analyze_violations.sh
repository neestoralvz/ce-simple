...
echo -e "${GREEN}PHASE_0_EMERGENCY VIOLATION ANALYSIS${NC}"
echo "Analyzing file size violations in context directory..."
...
analyze_violations() {
    echo -e "${YELLOW}Scanning all .md files for violations...${NC}"
    ...
    echo -e "${BLUE}VIOLATION SUMMARY:${NC}"
    echo -e "  ${RED}L0-EMERGENCY (≥400 lines): $l0_count files${NC}"
    echo -e "  ${YELLOW}L1-CRITICAL (200-399 lines): $l1_count files${NC}"
    echo -e "  ${GREEN}L2-HIGH (80-199 lines): $l2_count files${NC}"
    echo -e "  ${BLUE}TOTAL VIOLATIONS: $total_count files${NC}"
...
analyze_structure() {
    echo -e "${YELLOW}Analyzing file structures for extraction guidance...${NC}"
    
    # Create structure analysis for each violation level
    for level in "L0_EMERGENCY" "L1_CRITICAL" "L2_HIGH"; do
        echo -e "${BLUE}Analyzing $level files...${NC}"
        ...
        while read -r lines file; do
            if [[ -f "$file" ]]; then
                ...
                echo -e "  ${GREEN}✓${NC} $rel_path: $section_count sections, $subsection_count subsections"
            fi
...
generate_pilot_candidates() {
    echo -e "${YELLOW}Selecting L2-HIGH pilot test candidates...${NC}"
    ...
    echo -e "${BLUE}PILOT TEST CANDIDATES (5 files, 100-120 lines):${NC}"
    while read -r lines file; do
        rel_path="${file#$CONTEXT_DIR/}"
...
EOF
    ...
    echo -e "${GREEN}✓ Analysis report generated: $OUTPUT_DIR/VIOLATION_ANALYSIS_REPORT.md${NC}"
}
...
echo -e "${GREEN}🎉 VIOLATION ANALYSIS COMPLETED${NC}"
echo -e "${GREEN}Analysis results: $OUTPUT_DIR${NC}"
